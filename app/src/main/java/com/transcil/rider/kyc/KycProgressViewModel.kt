/**
 * ViewModel for the KYC progress accordion hub — the central screen listing all verification steps.
 * Fetches server onboarding state, renders step list, handles inline edits, Digio launch, and doc uploads.
 */
...
class KycProgressViewModel(application: Application) : AndroidViewModel(application) {
    ...
    // `private var`: holds picked file bytes between attach click and submit — cleared after upload.
     ...
     */
    /** `suspend fun` path: refresh pulls onboarding + reference contact in viewModelScope.launch. */
    ...
    /** `companion object`: pure helpers testable without a ViewModel instance. */
    companion object {
    ...
    /** `private fun`: rebuilds LiveData from KycProgressRepository after any local or server change. */
