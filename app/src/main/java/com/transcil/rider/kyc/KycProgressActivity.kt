/**
 * KYC progress hub — accordion UI listing all verification steps with inline forms and navigation.
 * Central screen riders return to after each step; supports browse-only mode from Profile → Documents.
 */
...
import com.transcil.rider.databinding.ActivityKycProgressBinding
import com.transcil.rider.databinding.ItemKycStepBinding
...
class KycProgressActivity :
    ...
    /** `companion object`: intent extra key for Profile → Documents browse mode. */
    companion object {
    ...
    // `private val`: computed from intent — browse mode disables auto-redirect to approved/pending.
    ...
    // ViewBinding map: one ItemKycStepBinding per KycStep for accordion row reuse.
    ...
    /** `override fun`: main hub — binds accordion steps, observes ViewModel, handles file pick results. */
