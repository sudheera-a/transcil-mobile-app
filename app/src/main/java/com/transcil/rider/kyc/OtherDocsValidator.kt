/**
 * Validates "Other Docs" step — Voter ID, Driving License, or PAN format and normalization.
 * Maps spinner labels to [OtherDocumentType] and returns field-level string-resource errors.
 */
package com.transcil.rider.kyc

import com.transcil.rider.R

/** `enum class`: document kinds offered in the Other Docs spinner. */
enum class OtherDocumentType {
    VOTER_ID,
    DRIVING_LICENSE,
    PAN_CARD;

    /** `companion object`: factory to resolve UI label → enum; returns null for unknown labels. */
    companion object {
...
/** `object`: regex-based validators for each document type. */
