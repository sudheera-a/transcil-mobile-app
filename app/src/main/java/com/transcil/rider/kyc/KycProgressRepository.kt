/**
 * In-memory KYC progress tracker and form drafts for the current onboarding session.
 * Holds step completion state, per-step drafts, and journey type until logout or journey switch.
 * Server truth is synced via [OnboardingSync]; local-only completion covers steps without APIs yet (e.g. bank).
 */
...
/** `data class`: immutable snapshot of personal-details form fields between screens. */
 ...
 * `object`: process-wide singleton — all KYC screens read/write the same repository.
 */
object KycProgressRepository {

    // `private var`: mutable journey pointer; null until rider picks Rent-EV or 3PL path.
    ...
    /** `private fun`: steps depend on current journey — empty when journey not yet chosen. */
