/**
 * Repository for manual KYC documents: presigned upload to S3, submit metadata, list status.
 * Orchestrates API steps (upload request → binary PUT → submit) that KYC Activities call as one operation.
 *
 * Kotlin notes:
 * - [withContext(Dispatchers.IO)] = run blocking OkHttp upload off the main thread inside a suspend fun.
 * - `private fun putBinary` = direct S3 PUT (not via Retrofit) using URL + headers from the API.
 * - `companion object` = [sha256Hex] for upload integrity and [apiDocType] UI label → API enum mapping.
 */
