/**
 * Repository for rider onboarding after login: journey, profile, address, references, PAN, geography.
 * Top-level extension functions map between core enums/UI date format and API string conventions.
 *
 * Kotlin notes:
 * - Top-level [fun JourneyType.toRiderRole] = extension function (adds method to enum without subclassing).
 * - `suspend fun` per endpoint; [runCatching] converts API errors to [Result.failure].
 * - [require] / [error] inside runCatching throw and become Result failures.
 */
