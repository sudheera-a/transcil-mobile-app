/**
 * Splash / launcher screen: short animation, then routes via [AuthSession.resolveColdStart].
 *
 * Binding notes (see also [com.transcil.rider.core.BaseActivity]):
 * - Extends BaseActivity with ActivityMainBinding — `binding.btnRetry` is type-safe UI access.
 * - `private val handler` = read-only Handler reference (cannot reassign `handler = ...`).
 * - `private var navigated` = mutable flag (can reassign) to avoid double navigation.
 * - [lifecycleScope.launch] = start a coroutine tied to this Activity's lifecycle
 *   (cancelled if the Activity is destroyed) — required to call `suspend` functions.
 * - `override fun onDestroy` = cleanup (remove delayed Runnable so it doesn't fire later).
 */
package com.transcil.rider.splash
...
import com.transcil.rider.databinding.ActivityMainBinding
