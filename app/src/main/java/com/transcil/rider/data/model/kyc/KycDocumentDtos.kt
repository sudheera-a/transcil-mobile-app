/**
 * DTOs for manual KYC document upload, submit, list, reference contact, and PAN verification.
 * Supports the presigned-URL upload flow (request URL → PUT bytes → submit metadata).
 *
 * Kotlin notes:
 * - [Map<String, String>] on requiredHeaders = S3 signing headers returned by the API.
 * - Default values (`emptyList()`, `false`) give safe fallbacks when JSON omits fields.
 */
