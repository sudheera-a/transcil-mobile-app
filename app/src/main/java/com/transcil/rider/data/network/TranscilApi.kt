/**
 * Retrofit API surface for the Transcil backend — one method per HTTP endpoint.
 *
 * Annotation cheat-sheet (Retrofit):
 * - `@GET` / `@POST` / `@PUT` / `@PATCH` = HTTP method + relative path.
 * - `@Body` = JSON request body (Gson serializes the Kotlin object).
 * - `@Header` = single HTTP header (e.g. Idempotency-Key).
 * - `@Path` = fill `{stateCode}` (etc.) in the URL path.
 * - `suspend fun` = coroutine-friendly; call from a coroutine, not from the main thread directly.
 *
 * Return type [ApiResponse] wraps success `data` or API `error`.
 */
