/**
 * Central navigation for post-submission KYC status screens and the home dashboard.
 * Maps server document status to [KycPendingActivity], [KycApprovedActivity], or [HomeDashboardActivity].
 *
 * Kotlin notes:
 * - `object` = app-wide helper with no mutable state; call KycNavigator.openForStatus(...).
 * - `const val` in companion-less object = compile-time constant for Intent extras.
 */
...
import com.transcil.rider.kyc.KycApprovedActivity
import com.transcil.rider.kyc.KycPendingActivity
